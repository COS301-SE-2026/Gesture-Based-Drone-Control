...
<div class="tx-badges">
  ...
  <span class="tx-status">For · First-time operators</span>
  <span class="tx-status">Reading time · ~10 min</span>
</div>

!!! abstract "Welcome"
    Welcome to the **Gesture-Based Drone Control System** — *GBDCS* for
    short. This is the system that lets you fly a drone with your hand
    instead of a remote control. You stand in front of your computer's
...
| [§6 Replay a Past Flight](#6-replay-a-past-flight) (UC-5) | You want to look back at something that happened earlier. |
...
- **A computer** — Windows, Mac, or Linux. A laptop with a built-in
  camera works fine.
...
- **About one metre of clear space** — the drone needs room to fly.
...
Any time you want the drone to do something. This is the only way to
control the drone — there is no joystick, no buttons.

### 3.3 How to use it

1. Make sure you've signed in (§2).
2. Make sure a drone or simulator is connected. The top-right of the
   dashboard will say **READY** when the system is ready to fly.
3. Hold your hand up in front of the camera. You should see a faint
...
6. Try other gestures: an open palm to keep hovering, a pointing
   finger to move in that direction, a fist to land.
7. To stop the drone immediately, lower your hand and click the red
   **Stop now** button on the dashboard.

### 3.4 What you should see

The middle of the dashboard shows your camera feed in real time, with
a thin coloured skeleton drawn on your hand. The badge at the
top-right shows the **current gesture** the system thinks you're
...
also pops up loud alerts on its own if anything important changes
(low battery, lost connection), so you don't have to stare at it.

### 4.3 How to use it

Just look at the right-hand side of the dashboard. There's nothing to
click. The numbers update on their own about five times a second
while you fly.
...
| **Flight mode** | What the drone is currently doing — `MANUAL`, `HOVER`, `LANDING`, `EMERGENCY`. | It changes to `EMERGENCY` without you pressing Stop. |
| **Link** | The connection between the drone and your computer. Either `OK` or `LOST`. | It says `LOST` for more than a second or two. |
...
is that the drone is on screen instead of in the air.
...
- After making a mistake on a real drone, so you can practise without
  risking another crash.

### 5.3 How to use it

1. Start AirSim. It opens a window with a flying environment.
2. In the GBDCS app, before signing in or before starting a session,
   look for the **Adapter** dropdown in the dashboard settings.
3. Choose **AirSim** from the dropdown.
4. Start the session as normal.
5. Fly as you would in §3 — every gesture works the same way.
...
## 6. Replay a Past Flight &nbsp;<small>UC-5</small>
...
back. You can play any of these back later in **replay mode**.
...
- For evaluation — your academic and industry mentors can review
  recorded demo runs.

### 6.3 How to use it

1. From the main dashboard, click **Replay** in the top menu.
...
4. Click **Play**.
5. The dashboard reconstructs the flight: the camera feed, the hand
   skeleton, the gestures, and the telemetry, all playing back at
   the original speed.
...
Every screen has a **?** icon in the top-right corner. Click it to
open the in-app help menu without leaving the dashboard.
...
The help menu does not interrupt your flight — you can open it while
the drone is hovering and close it again to resume control.
...
| `Ctrl + .` keyboard shortcut | Works from anywhere on the dashboard. |

!!! warning "When in doubt, hit Stop now"
    No one is graded on landing gracefully. If something feels wrong,
    use **Stop now** — that's exactly what it's there for.
...
- Check the dashboard status — it must say **READY** before take-off
  works.
...
- Re-sign-in (§2). The replay log (§6) will still have everything
