...
export default function PacDroneGame() {
  ...
  useKaplayCanvas(canvasRef, (k, fonts) => {
    ...
    k.scene("title", () => {
      k.add([
        k.text("PAC-DRONE", { size: 105, font: fonts.heading }),
        k.anchor("center"),
        k.pos(w / 2, h / 2 - 120),
        k.color(...col_power),
      ])
      ...
      k.add([
        k.text("Choose a maze:", { size: 55, font: fonts.body }),
        k.anchor("center"),
        k.pos(w / 2, h / 2 - 30),
        k.color(...col_wall),
      ])
    ...
    k.scene("game", (mazeIndex = 0) => {
      ...
      const scoreLbl = k.add([
        k.text(`SCORE ${score}`, { size: 22, font: fonts.mono }),
        k.anchor("topleft"),
        k.pos(8, 3),
        k.color(...col_player),
        k.fixed(),
      ...
      const statusLbl = k.add([
        k.text("", { size: 17, font: fonts.mono }),
        k.anchor("topright"),
        k.pos(w - 8, 8),
        k.color(...col_power),
        k.fixed(),
      ...
      k.onUpdate(() => {
...

        {
          ...
          k.get("pellet").forEach((p) => {
            if (p.col === playerCol && p.row === playerRow) {
              ...
              statusLbl.text = "GET YOUR REVENGE :D"
            }
          })
        }
        ...
        if (scared) {
          ...
          if (scaredTimer <= 0) {
            ...
            ghosts.forEach(
              (g) =>
                (g.color = flash ? k.rgb(...col_scared) : k.rgb(200, 200, 255))
            )
          }
        }
    ...
    k.scene("win", (score = 0, nextMaze = 0) => {
      k.add([
        k.text("YOUR'E WINNER!", { size: 122, font: fonts.heading }),
        k.anchor("center"),
        k.pos(w / 2, h / 2 - 80),
        k.color(...col_power),
      ])
      ...
      k.add([
        k.text("Next", { size: 26, font: fonts.mono }),
        k.anchor("center"),
        k.pos(w / 2, h / 2 + 270),
        k.color(...col_player),
      ])

      k.wait(10.5, () => k.go("game", nextMaze))
    })
    ...
    k.scene("lose", (score = 0, mazeIndex = 0) => {
      ...
      k.add([
        k.text("Enter / Space to retry  |  T for title", {
          size: 26,
          font: fonts.mono,
        }),
        k.anchor("center"),
