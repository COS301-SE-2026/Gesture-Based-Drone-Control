...
async def test_keyboard_keydown_mapping_all_keys():
	...
	for key, expected in KEY_MAP.items():
		await adapter.handle_message({'key': key, 'event': 'keydown'})
...
async def test_keyboard_ignores_keyup_events():
	...
	await adapter.handle_message({'key': 't', 'event': 'keyup'})
	await adapter.handle_message({'key': 'ArrowUp', 'event': 'keyup'})
...
async def test_keyboard_handles_missing_key_field():
	...
	await adapter.handle_message({'event': 'keydown'})  # missing key
...
async def test_keyboard_handles_non_dict_input(caplog):
	...
	await adapter.handle_message(None)  # NOSONAR
