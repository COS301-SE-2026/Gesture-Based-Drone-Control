...
def _discover(root: pathlib.Path, glob: str, what: str, override: str, keep) -> pathlib.Path:
	...
	if len(matches) > 1:
		...
		raise SimLaunchError(f'Ambiguous {what} under {root}: {listed}. Set the override in .env')
...
class PixelStreamLauncher:
	...
	def _check_bootstrap(self) -> pathlib.Path:
		...
		if shutil.which(self._s.pas_node) is None:
			raise SimLaunchError(
				f'{self._s.pas_node!r} is not on PATH - node is needed to run the signalling server'
			)
	...
	async def _start_sim(self, binary: pathlib.Path) -> None:
		...
		await self._await_port(self._s.pas_services_port, SIM_READY_TIMEOUT_S, self._sim, 'sim')
